...
@interface YBDatePickerView ()<UIPickerViewDataSource, UIPickerViewDelegate>
/** 选择的年 */
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view {
    NSString *text;
    if (component == 0) {
        text =  [NSString stringWithFormat:@"%ld年", (long)(row + _minShowYear)];
    } else if (component == 1) {
        text =  [NSString stringWithFormat:@"%02zd月", (NSInteger)(row + 1)];
    } else {
        text = [NSString stringWithFormat:@"%02zd日", (NSInteger)(row + 1)];
    }
